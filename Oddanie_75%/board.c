...
struct board *init_board(int forPlayer)
{
	...
	int i;
	for(i=0;i<3;i++) //tworzenie statkow
	{
...
//funkcja przygotowuje plansze do wyslania przez siec poprzez wrzucenie jej do stringa
...
//funkcja odczytuje zapis tablicy ze stringa przekazanego poprzez siec
int responseToBoard(struct board *b,char *response)
{
	...
	for(i=0;i<BOARDSIZE;i++)
		for(j=0;j<BOARDSIZE;j++)
		{
			sscanf(response,"%d",&b->table[j][i]);//odczytujemy pierwsza cyfre ze stringa
