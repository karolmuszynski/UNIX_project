...
void parseResponse(struct game *g,char* response,char *request)
{
		...
		{			
			...
			int fx,fy,tx,ty,mt;
			printf("Enter number (x,y)->(tx,ty) mt (enter -1 to disconnect)");
			scanf("%d",&fx); //rozlaczanie gracza w trakcie rozgrywki
			if(fx==-1) 
			{
			...
			{
				scanf("%d %d %d %d",&fy,&tx,&ty,&mt); //prosimy gracza o nowy ruch
